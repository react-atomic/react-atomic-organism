...
function findType2Conflicts(g, layering) {
  ...
  function scan(south, southPos, southEnd, prevNorthBorder, nextNorthBorder) {
    var v;
    range(southEnd).slice(southPos).forEach( function(i) {
      v = south[i];
      if (g.node(v).dummy) {
...
function horizontalCompaction(g, layering, root, align, reverseSep) {
  ...
  keys(align).map(key=>align[key]).forEach( 
    v => xs[v] = xs[root[v]]
  );
...
function findSmallestWidthAlignment(g, xss) {
  ...
  const arr = keys(xss).map(key=>xss[key]).map( function (xs) {
...
function balance(xss, align) {
  const callback = function(ignore, v) {  
    if (align) {
      return xss[align.toLowerCase()][v];
    } else {
      const xs = keys(xss).map(key=>xss[key][v]).sort()
      return (xs[1] + xs[2]) / 2;
    }
  }
  const result = {}
  keys(xss.ul).forEach(key => result[key] = callback(xss.ul[key], key)) 
  return result
}
...
function positionX(g) {
  ...
  var xss = {},
      adjustedLayering;
  ["u", "d"].forEach( function(vert) {
    adjustedLayering = vert === "u" ? layering : keys(layering).map(key=>layering[key]).reverse();
    ["l", "r"].forEach( function(horiz) {
      if (horiz === "r") {
        adjustedLayering = adjustedLayering.map( function(inner) {
          return keys(inner).map(key=>inner[key]).reverse();
        });
      }
