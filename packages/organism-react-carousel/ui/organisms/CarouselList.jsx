...
class CarouselList extends Component {
  render() {
    ...
    if (carouselAttr) {
      thisChildren = React.Children.map(children, (child) => {
        return React.cloneElement(child, { ...carouselAttr });
      });
    }
    if (innerContainer) {
      thisChildren = React.cloneElement(innerContainer, {}, thisChildren);
    }
    // Child -->
...
const Styles = {
  ...
  arrowLeft: {
    left: 0,
    backgroundImage:
      "linear-gradient(to right, rgba(0,0,0,0.3) 0, rgba(0,0,0,0.001) 100%)",
  },
  arrowRight: {
    right: 0,
    backgroundImage:
      "linear-gradient(to right, rgba(0,0,0,0.001) 0, rgba(0,0,0,0.3) 100%)",
  },
  icon: {
    width: 50,
    height: 50,
