...
const defaultUnHandle = field =>  (other, key) =>
{
    ...
    if (nodeName.toUpperCase() === 'SCRIPT') {
        ...
        return (
            <Unsafe key={key}>{outerHTML}</Unsafe>
        );
    }
};
            ...
            <Content style={Styles.content}>
                {
                    fields.map( (field, key) => {
                        ...
                        if (field.others) {
                            ...
                            others = field.others.map(
                                (other, key)=> unHandler(other, key)
                            );
                        }
                        if (field.helps) {
                            thisHelp = <Description style={Styles.help}>{field.helps}</Description>;
                        }
                        return (
                            <SemanticUI className={rowClassName.join(' ')} key={key} style={Styles.row}>
                                {rowLabel}
                                {fieldChild}
                                {others}
                                {thisHelp}    
